...
def process_single_shot(couplings_bin, fields_bin, identity, qubits, qudits, a, G):
    """Runs a single shot of the LQA algorithm and returns success metrics and config."""
    machine = ExcLQA.ExcLqa(couplings_bin, fields_bin, identity, qubits, qudits, a)
    machine.minimise(step=0.999, N=100, g=8, f=0.2)
    ...
    for k in range(100):
        ...
        if valid(shortest_vector, qubits) or valid(-shortest_vector, qubits):
            ...
            with ThreadPoolExecutor() as executor:
                futures = [
                    executor.submit(process_single_shot, couplings_bin, fields_bin, identity, qubits, qudits, a, G)
